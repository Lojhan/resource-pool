 ...
 */
function calculateSlope(yValues: number[], xValues: number[]): number {
...
async function runLeakTest(mode: 'sync' | 'async') {
  ...
  console.log(`Pool initialized with ${pool.getMetrics().available} resources.`);
  ...
  for (let i = 0; i < WARMUP_ITERATIONS; i++) {
    ...
    if (!res) throw new Error('Failed to acquire resource during warm-up');
    res.id++;
    pool.release(res);
  }
  ...
  for (let i = 0; i <= ITERATIONS; i++) {
    const res = mode === 'async' ? await pool.acquireAsync() : pool.acquire();
    if (!res) throw new Error('Failed to acquire resource during main loop');
    res.id++;
    pool.release(res);
